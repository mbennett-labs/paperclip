...
# Finalize QSL-1 Flight #2 workspace after the prior recovery proved the clone
# exists but accidentally verified its HEAD as root. The workspace is owned by
# OpenClaw, so every git operation against it must run as OpenClaw. This script
# makes no source edits, does not restart services, does not wake QSL-1, and
...
MISSION_HEAD="$(runuser -u "$OPENCLAW_USER" -- git -C "$MISSION_WORKSPACE" rev-parse HEAD)"
[[ "$MISSION_HEAD" == "$HEAD_NOW" ]] || fail "mission workspace HEAD mismatch: workspace=$MISSION_HEAD reliability=$HEAD_NOW"
