...
# QSL Mission Control V0.1 — verifier diagnostics/evidence repair gate.
#
# Applies the already-reviewed reliability commit that makes failed verification
# stages emit bounded diagnostic output and records tests/typecheck/build results
# independently in verification evidence. Refreshes the isolated QSL-1 Mission
# Cell workspace without giving it a remote. No mission retry and no production
# mutation occur here.
...
for cmd in git curl jq systemctl runuser mktemp chmod rm pnpm bash; do need "$cmd"; done
...
REMOTE_HEAD="$(git rev-parse "origin/$WORK_BRANCH")"
[[ "$REMOTE_HEAD" == "$TARGET_HEAD" ]] || fail "unexpected reliability head: $REMOTE_HEAD (expected $TARGET_HEAD)"
git merge --ff-only "origin/$WORK_BRANCH"
[[ "$(git rev-parse HEAD)" == "$TARGET_HEAD" ]] || fail "local reliability head did not reach target"

echo "Reliability HEAD: $TARGET_HEAD"

# Force Node 22 for pnpm/vitest without changing host-wide node configuration.
...
pnpm exec vitest run --config scripts/operator-loop/vitest.config.ts \
  scripts/operator-loop/__tests__/authority-policy.test.ts \
  scripts/operator-loop/__tests__/verify-mission-output.test.ts

# The behavioral regression must prove both diagnostics visibility and per-stage
# evidence; do not accept a static source-only assertion.
...
# QSL-1 remains deliberately blocked until this repair gate is proven.
