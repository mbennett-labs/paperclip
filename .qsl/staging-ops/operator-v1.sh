...
#
# NOTE: this function MUST be defined BEFORE the `case "$op"` dispatch below.
# Bash executes top-to-bottom, so a definition placed after the case arm that
# invokes it would fail at runtime with "command not found" — a defect that
# `bash -n` cannot detect. The operator runtime test
# (.qsl/staging-ops/operator-runtime-test.sh) executes this operation against
# a real bash and guards against regressions.
