...
export function decideConversationPolicy(input: ConversationPolicyInput): ConversationPolicyDecision {
  ...
  if (input.tenant === "therapist_index" && (input.intent === "listing_removal" || input.intent === "correction")) {
    return {
      state: "human_review",
      ...
      nextAction: {
        ...
        reason: "TherapistIndex removals/corrections require stronger identity and evidence review before profile changes.",
        humanApprovalRequired: true,
      },
    };
  }
